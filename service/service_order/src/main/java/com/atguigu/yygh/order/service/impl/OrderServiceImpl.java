...
public class OrderServiceImpl extends ServiceImpl<OrderMapper, OrderInfo> implements OrderService {
    ...
    public Boolean cancelOrder(Long orderId) {
        ...
        if (result.getInteger("code") != 200) {
            ...
            //判断当前订单是否可以取消（OrderStatusEnum.PAID：已支付）
            if (orderInfo.getOrderStatus().intValue() == OrderStatusEnum.PAID.getStatus().intValue()) {
                ...
                //TODO 判断当前订单是否可以取消（OrderStatusEnum.UNPAID：预约成功，待支付）
//                if ((orderInfo.getOrderStatus().intValue() == OrderStatusEnum.UNPAID.getStatus().intValue())) {
//                    Schedule schedule = scheduleRepository.getScheduleByHoscodeAndHosScheduleId(orderInfo.getHoscode(), orderInfo.getScheduleId());
//                    schedule.setAvailableNumber(schedule.getReservedNumber()+1);
//                }
            }
        }
        return true;
